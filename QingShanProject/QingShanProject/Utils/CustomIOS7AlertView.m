...
//    dialogView.frame = CGRectMake(0, kDeviceHeight, dialogView.frame.size.width, dialogView.frame.size.height);
...
//                         dialogView.frame = CGRectMake(0, kDeviceHeight-dialogView.frame.size.height, dialogView.frame.size.width, dialogView.frame.size.height);
...
- (void)showFromBottom{
    dialogView = [self createContainerView : kDeviceHeight];
    dialogView.frame = CGRectMake(0, kDeviceHeight, dialogView.frame.size.width, dialogView.frame.size.height);
...
#pragma mark 可以让调用者自己调整ContainerView的上下高度（调用creatContainerView : ）
- (void)showWithY : (CGFloat)yHeight
{
    dialogView = [self createContainerView : yHeight];
...
- (void)showWithY : (CGFloat)yHeight X:(CGFloat)xWidth{
    dialogView = [self createContainerView : yHeight :xWidth];
...
#pragma mark 可以让调用者自己调整ContainerView的上下高度（被show调用）
- (UIView *)createContainerView : (CGFloat)yHeight
{
    if (containerView == NULL) {
        containerView = [[UIView alloc] initWithFrame:CGRectMake(0, 0, 300, 150)];
    }
...
- (UIView *)createContainerView : (CGFloat)yHeight :(CGFloat)xWidth
{
    if (containerView == NULL) {
        containerView = [[UIView alloc] initWithFrame:CGRectMake(0, 0, 300, 150)];
    }
    ...
    [self addGestureRecognizer:tapGesture];
    // This is the dialog's container; we attach the custom content and the buttons to this one
    UIView *dialogContainer = [[UIView alloc] initWithFrame:CGRectMake(xWidth, yHeight, dialogSize.width, dialogSize.height)];//在此处把y设为了yHeight，仅改动了此处
